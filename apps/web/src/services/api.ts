...
export async function lookupAddress(address: string) {
  const res = await fetch(`${API_BASE}/api/v1/lookup/address?address=${encodeURIComponent(address)}`);
  if (!res.ok) throw new Error('Failed to lookup address');
  return res.json() as Promise<{ summary: AddressSummary; labels: LabelItem[]; risk: RiskEvaluation }>;
}

export async function fetchTraceGraph(seedAddress: string, maxHops: number = 2) {
  const res = await fetch(`${API_BASE}/api/v1/tracing/graph?seed_address=${encodeURIComponent(seedAddress)}&max_hops=${maxHops}`);
...
export async function createCase(data: { title: string; description: string; tags: string[] }) {
  const res = await fetch(`${API_BASE}/api/v1/cases`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export function getExportUrl(caseId: string, format: string = 'JSON') {
