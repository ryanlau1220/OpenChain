	...
	if (!summary) {
		return (
			<div className="p-12 text-center text-slate-500">
				<p className="text-sm">
					Enter a Sepolia testnet address above to view wallet metrics and
		...
		<div className="p-8 max-w-6xl mx-auto space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				...
				<div className="p-5 glass-panel rounded-2xl space-y-2 border-l-4 border-purple-500">
					...
					<span className="text-[10px] text-slate-500">
						{summary.label || 'Unlabeled EOA'}
					</span>
				</div>

				<div
					className={`p-5 glass-panel rounded-2xl space-y-2 border-l-4 ${summary.risk_score >= 50 ? 'border-red-500' : 'border-emerald-500'}`}
				>
					<div className="flex items-center justify-between text-xs text-slate-400">
						<span>Risk Score</span>
						<ShieldAlert
							className={`w-4 h-4 ${summary.risk_score >= 50 ? 'text-red-400' : 'text-emerald-400'}`}
						/>
					</div>
					<p
						className={`text-2xl font-bold font-mono ${summary.risk_score >= 50 ? 'text-red-400' : 'text-emerald-400'}`}
					>
			...
			<div className="p-6 glass-panel rounded-2xl flex items-center justify-between gap-4">
				<div>
					...
					<h2 className="text-lg font-bold font-mono text-white mt-1 break-all">
						{summary.address}
					</h2>
					{labels && labels.length > 0 && (
						<div className="flex flex-wrap gap-2 mt-2">
							{labels.map((l) => (
								<span
									key={l.id}
									className="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-cyan-500/20 text-cyan-300 border border-cyan-500/30"
								>
									{l.category}: {l.label}
								</span>
							))}
				...
				<div className="p-6 glass-panel rounded-2xl space-y-3">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
						{risk.flags.map((flag) => (
							<div
								key={flag.rule_id}
								className="p-4 bg-slate-900/80 rounded-xl border border-slate-800 space-y-1"
							>
