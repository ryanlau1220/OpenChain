...
import { ZoomIn, ZoomOut, RefreshCw, Maximize2, Layers, Filter, PlusCircle, ExternalLink } from 'lucide-react';
...
export const GraphCanvas: React.FC<GraphCanvasProps> = ({ graphData, onNodeSelect, onExpandNode }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const cyRef = useRef<Core | null>(null);
  const [selectedNode, setSelectedNode] = useState<GraphNode | null>(null);
  const [selectedEdge, setSelectedEdge] = useState<GraphEdge | null>(null);
  const [layoutName, setLayoutName] = useState<'cose' | 'concentric' | 'breadthfirst' | 'grid'>('cose');
  ...
  useEffect(() => {
    ...
    graphData.nodes.forEach((n) => {
      ...
      if (n.is_seed) bg = '#06B6D4'; // Seed Cyan
      else if (n.risk_score >= 50) bg = '#EF4444'; // Red Risk
      ...
      style: [
        {
          ...
          style: {
            ...
            'text-valign': 'bottom',
            'text-margin-y': 6,
            'width': (ele: any) => (ele.data('is_seed') ? 48 : 36),
            'height': (ele: any) => (ele.data('is_seed') ? 48 : 36),
            'border-width': (ele: any) => (ele.data('is_seed') ? 3 : 1),
            'border-color': '#FFFFFF',
            'overlay-padding': '4px',
          },
        },
    ...
    <div className="relative w-full h-[calc(100vh-7rem)] bg-slate-950 flex flex-col overflow-hidden">
      ...
      {graphData && (
        <div className="absolute top-4 right-4 z-10 flex items-center gap-4 px-4 py-2 glass-panel rounded-xl text-xs text-slate-300">
          <div>
            Seed: <span className="font-mono font-semibold text-cyan-400">{graphData.seed_address.substring(0, 10)}...</span>
          </div>
          <div className="h-3 w-[1px] bg-slate-800" />
          <div>Nodes: <span className="font-bold text-white">{graphData.total_nodes}</span></div>
          <div>Edges: <span className="font-bold text-white">{graphData.total_edges}</span></div>
        </div>
      )}
        ...
        <div className="absolute bottom-4 left-4 z-10 w-96 p-4 glass-panel rounded-2xl shadow-2xl border border-cyan-500/30 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="flex justify-between items-center py-1">
              ...
              <span className="font-semibold text-slate-200">{selectedNode.entity_type}</span>
            </div>
            <div className="flex justify-between items-center py-1">
              <span className="text-slate-400">Risk Score:</span>
              <span className={`font-bold ${selectedNode.risk_score >= 50 ? 'text-red-400' : 'text-emerald-400'}`}>
