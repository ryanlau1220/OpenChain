			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-xs text-slate-400">
						WebSocket feed powered by coder/websocket.
					</p>
				</div>
				...
				>
					<Radio
						className={`w-3.5 h-3.5 ${wsConnected ? 'animate-pulse' : ''}`}
					/>
				...
				<div className="bg-slate-950 p-4 rounded-xl font-mono text-xs text-slate-300 space-y-2 border border-slate-900 max-h-96 overflow-y-auto">
					{messages.length === 0 ? (
						...
						messages.map((m, idx) => (
							<div
								key={idx}
								className="py-1 border-b border-slate-900 last:border-0 flex items-center gap-2"
							>
